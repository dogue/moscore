...
fn bmi_false() {
    let bus = MockBus::new();
    let program = vec![0x30, 0x05];
    let mut core = Core::new(bus, program).unwrap();
    core.status.set_negative(false);
    core.step();
...
#[test]
fn bmi_negative_offset_page_crossed() {
    let mut bus = MockBus::new();
    bus.write(0x8000, 0x30);
