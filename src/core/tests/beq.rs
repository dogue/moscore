...
fn beq_false() {
    let bus = MockBus::new();
    let program = vec![0xF0, 0x05];
    let mut core = Core::new(bus, program).unwrap();
    core.status.set_zero(false);
    core.step();
...
#[test]
fn beq_negative_offset_page_crossed() {
    let mut bus = MockBus::new();
